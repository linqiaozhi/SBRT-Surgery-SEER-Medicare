...
sim  <-  function( N = 1E5, eps_W = 1, eps_Z=1, rateC = 0.01, lambda = 0.015, beta_A = 0.001, beta_U = 0.001,  verbose = F , plotit=F)  { 
 ...
 # plotit=T
    ...
    W  <-  U + eps_W*rnorm (N) -10* X    # proxy 1
    Z  <-  U + eps_Z*rnorm (N)  +3* X   # proxy 2
    ...
    stage1  <-  lm( W ~ A + Z  , data =  data.sim )
    # W_hat_Zonly  <- coef(stage1)['Z']*data.sim$Z
    # W_hat_Zonly  <- coef(stage1)[c('Z', 'A')] %*% t(matrix( data.sim$Z, data.sim$A, ncol=2, byrow=T))
    W_hat_Zonly  <-  predict(stage1, data.sim)
    # W_hat_Zonly  <- as.matrix( data.sim[,c('A','Z')]) %*% coef(stage1)[c('Z', 'A')] 
    data.sim.2  <-  data.frame( data.sim, W_hat = W_hat_Zonly)
    kfit  <-  aalen( Surv(outcome.time, outcome.observed) ~ 1+ const(A) + const(X) + const(W_hat)  ,  data = data.sim.2, robust = 0)
    if (verbose) print(summary(kfit))
    out[['without.u.two.stage.Zonly']]  <-  calc.err( kfit, beta_A)
    return(out)
}
...
print.results  <-  function ( outs) {
    ...
    print(sprintf('Aalen\'s with two stage: %.1f%% +/- %.1f%% error', out.means['without.u.two.stage.Zonly'], out.se['without.u.two.stage.Zonly']))
}
